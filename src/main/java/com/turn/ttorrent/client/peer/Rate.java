 ...
 *
 * <p>
 * This is a utility class to keep track, and compare, of the data exchange rate (either download or
 * upload) with a peer.
 * </p>
 *
 * @author mpetazzoni
...
public class Rate implements Comparable<Rate> {
   ...
   */
  public synchronized void add(final long count) {
    this.bytes += count;
    if (this.reset == 0) {
   ...
   *
   * <p>
   * This class provides a comparator to sort peers by an exchange rate, comparing two rates and
   * returning an ascending ordering.
   * </p>
   *
   * <p>
   * <b>Note:</b> we need to make sure here that we don't return 0, which would provide an ordering
   * that is inconsistent with <code>equals()</code>'s behavior, and result in unpredictable
   * behavior for sorted collections using this comparator.
   * </p>
   *
   * @author mpetazzoni
   */
  private static class RateComparator implements Comparator<Rate>, Serializable {
     ...
     *
     * <p>
     * This method compares float, but we don't care too much about rounding errors. It's just to
     * order peers so super-strict rate based order is not required.
     * </p>
     *
     ...
     */
    @Override
    public int compare(final Rate a, final Rate b) {
